...
def _resnet(
    ...
    **kwargs
    ...
    if pretrained:
        model.load_state_dict(torch.load(r'resnet18-5c106cde.pth'))
    return model
...
class Net(nn.Module):
    def __init__(self, num_classes=config.class_num):
        ...
        self.fc = nn.Linear(512, config.features_dim)
        self.gc1 = GraphConvolution(config.features_dim, config.features_dim)
        self.gc2 = GraphConvolution(config.features_dim, config.features_dim)
...
def extractor(pre_training=False):
    model = resnet50()
    if pre_training:
        model = torch.load(r'resnet18-5c106cde.pth')
        model.load_state_dict(torch.load(r'resnet50-19c8e357.pth'))
    return model
